...
it("loads account-owned history only on demand and never sends a mutation", async () => {
  ...
  state.request.mockResolvedValueOnce({ scale: 1000, receipt: { id: "import-one", wallet: "0xoriginal-wallet", credit_units: 2125,
    jobs: [{ id: "job-one" }], publication_ids: ["song-one"], playlist_ids: ["playlist-one"], workflow_ids: ["12"], like_ids: ["liked-one"], save_ids: ["saved-one"] } });
  ...
  expect(host.textContent).toContain("Liked song: liked-one");
  expect(host.textContent).toContain("Saved song: saved-one");
