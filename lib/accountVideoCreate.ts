...
export async function submitAccountCreateVideo(storage: Storage, account: string, access: AccountStudioAccess, input?: VideoJobRequest): Promise<V1Job> {
  ...
  const body: Record<string, unknown> = { type: input.initImage ? "image_to_video" : "text_to_video",
