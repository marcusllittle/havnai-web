...
import { AccountWorkflowLibrary } from "../components/AccountWorkflowLibrary";
import { fetchMarketplace, type Workflow } from "../lib/havnai";
...
function TemplateDetails({ workflow, onClose }: { workflow: Workflow; onClose: () => void }) {
  ...
  return <dialog ref={dialog} className="template-dialog" aria-labelledby="template-title" onCancel={onClose} onClick={event => { if (event.target === event.currentTarget) onClose(); }}>
    <div className="template-detail">
      ...
      {workflow.creator_wallet && <section><h3>Creator</h3><p className="template-wallet">{workflow.creator_wallet}</p></section>}
...
export default function TemplatesPage() {
  ...
  return <TemplatesContent key={account.account?.id || "guest"} />;
}

function TemplatesContent() {
  const account = useAccount();
  const [editing, setEditing] = useState<Workflow | null>(null);
  const pendingSave = useRef<{ key: string; body: string } | null>(null);
  ...
  const identity = useRef(account.account?.id);
  identity.current = account.account?.id;
  ...
  useEffect(() => {
    ...
    fetchMarketplace({ accountCatalog: true, search: query || undefined, category: category || undefined, offset: page * 12, limit: 12, signal: controller.signal })
      .then(result => {
  ...
  const save = async (event: FormEvent) => {
    ...
    const address = account.account?.id;
    if (saveInFlight.current || !address) return;
    ...
    try {
      const body = JSON.stringify({ name: draft.name.trim(), description: draft.description.trim(), category: draft.category, config: { ...editing?.config, model: draft.model.trim() || "auto", prompt_template: draft.prompt.trim(), negative_prompt: draft.negative.trim(), steps, guidance } });
      if (!pendingSave.current || pendingSave.current.body !== body) pendingSave.current = { key: crypto.randomUUID(), body };
      const result = await account.request<Workflow>(editing ? `/v2/account/workflows/${encodeURIComponent(editing.id)}` : "/v2/account/workflows", {
        method: editing ? "PATCH" : "POST", body, signal: controller.signal, headers: { "Idempotency-Key": pendingSave.current.key } });
  ...
  function edit(workflow: Workflow) {
    ...
    setEditing(workflow); pendingSave.current = null; setSaved(null); setSaveError(""); setView("create");
    setDraft({ name: workflow.name, description: workflow.description || "", category: workflow.category || "Image Generation",
      model: String(workflow.config.model || ""), prompt: String(workflow.config.prompt_template || ""), negative: String(workflow.config.negative_prompt || ""),
      steps: String(workflow.config.steps ?? 28), guidance: String(workflow.config.guidance ?? 6) });
  }
  return <>
    ...
    <main className="product-page templates-page">
      ...
      {view === "browse" ? <>
        ...
        {loading ? <div className="templates-state" role="status"><Layers3 size={28} aria-hidden="true" /><h3>Finding your next starting point…</h3></div> : error ? <div className="templates-state" role="alert"><h3>We couldn’t reach the catalog.</h3><p>{error}</p><button className="product-secondary" type="button" onClick={() => setRevision(value => value + 1)}>Try again</button></div> : items.length === 0 ? <div className="templates-state"><Layers3 size={30} aria-hidden="true" /><h3>{query || category ? "No matching templates yet." : "A little room for inspiration."}</h3><p>{query || category ? "Try a different search or category." : "The shared catalog is empty. You can still save your own starting point."}</p>{query || category ? <button className="product-secondary" type="button" onClick={() => { setSearch(""); setQuery(""); setCategory(""); setPage(0); }}>Clear filters</button> : <button className="product-secondary" type="button" onClick={() => setView("create")}>Create a template</button>}</div> : <>
          <div className="templates-grid">{items.map(item => <button type="button" className="template-card" key={item.id} onClick={() => setSelected(item)}><div className="template-card-top"><Layers3 size={24} aria-hidden="true" /><span>{item.category || "Other"}</span></div><h3>{item.name}</h3><p>{item.description || "A reusable setup, ready for your own direction."}</p><div className="template-card-model">{String(item.config?.model || "Auto model")}</div><div className="template-card-bottom">{typeof item.usage_count === "number" && <span>{item.usage_count} uses</span>}<strong>View template <ArrowRight size={15} aria-hidden="true" /></strong></div></button>)}</div>
      ...
      </> : <div className="template-compose"><aside><span className="product-eyebrow">Keep a good idea close</span><h2>A setup worth coming back to.</h2><p>Save the prompt and settings you want to reuse. You can review the template in Create before generating.</p><div><SlidersHorizontal size={22} aria-hidden="true" /><p>Saving a new template keeps it private. Publish it from your saved templates when you want to share its prompt and settings.</p></div></aside><form onSubmit={save}><h2>Your template</h2><fieldset disabled={saving}><label>Name<input required maxLength={160} value={draft.name} onChange={e => update("name", e.target.value)} placeholder="Golden-hour portraits" /></label><label>Description<textarea rows={2} maxLength={2000} value={draft.description} onChange={e => update("description", e.target.value)} placeholder="What makes this setup useful?" /></label><label>Category<select value={draft.category} onChange={e => update("category", e.target.value)}>{categories.map(item => <option key={item}>{item}</option>)}</select></label><label>Prompt template<textarea rows={5} maxLength={4000} value={draft.prompt} onChange={e => update("prompt", e.target.value)} placeholder="An editorial portrait of {subject}, warm afternoon light…" /></label><p className="template-hint">Use placeholders such as {"{subject}"} to remind yourself what to edit.</p><details className="template-settings"><summary>Model &amp; generation settings</summary><label>Model<input value={draft.model} maxLength={200} onChange={e => update("model", e.target.value)} placeholder="Auto: choose from available models" /></label><label>Negative prompt<textarea rows={2} maxLength={4000} value={draft.negative} onChange={e => update("negative", e.target.value)} placeholder="What should the result avoid?" /></label><div className="template-number-fields"><label>Steps<input type="number" required min={1} max={100} value={draft.steps} onChange={e => update("steps", e.target.value)} /></label><label>Guidance<input type="number" required min={1} max={30} step={0.5} value={draft.guidance} onChange={e => update("guidance", e.target.value)} /></label></div></details></fieldset>
        {editing && <p>Editing {editing.name}.{editing.published ? " Changes will also update the published template." : " This template is private."}</p>}
        <button className="product-secondary" type="button" disabled={saving} onClick={() => { setEditing(null); setDraft(emptyDraft); setSaved(null); setSaveError(""); pendingSave.current = null; }}>Start a new template</button>
        {saveError && <p className="template-error" role="alert">{saveError} Refresh your saved templates to check whether the save completed before starting a new template.</p>}
        {saved && <div className="template-saved" role="status"><strong>“{saved.name}” is saved.</strong><Link href={workflowHref(`account:${saved.id}`)}>Review in Create <ArrowUpRight size={15} aria-hidden="true" /></Link></div>}
        {hasAccount ? <button className="product-primary" type="submit" disabled={saving || !!saved}>{saving ? "Saving…" : saved ? "Saved" : "Save template"}</button> : <><p className="template-hint">Sign in to save templates. No wallet is required.</p><Link className="product-primary" href="/sign-in?redirect_url=%2Ftemplates">Sign in</Link></>}
      </form></div>}
      {hasAccount && <AccountWorkflowLibrary key={`${account.account!.id}:${saved?.updated_at || ""}`} onEdit={edit} />}<footer className="templates-footer"><p>Templates are starting points. Results depend on the model and available capacity.</p><Link href="/create">Start from scratch <ArrowUpRight size={15} aria-hidden="true" /></Link></footer>
